...
import { 
  Box, Button, Checkbox, FormControl, MenuItem, Select, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, Typography, Paper, Divider
} from '@mui/material';
...
function ManualJournal() {
  ...
  const addRow = () => {
    setRows([...rows, { account: '', description: '', contact: '', debit: 0, credit: 0 }]);
  };
  ...
  return (
    <Box sx={{ padding: 4 }}>
      <Typography variant="h4" gutterBottom>New Journal</Typography>
                  ...
                  <FormControl fullWidth>
                    ...
                    >
                      ...
                      {contacts.map((contact, i) => (
                        <MenuItem key={i} value={contact}>{contact}</MenuItem>
                      ))}
                    </Select>
                  </FormControl>
                </TableCell>
      ...
      <Button variant="contained" color="primary" onClick={addRow} sx={{ mb: 2 }}>Add New Row</Button>

      <Divider sx={{ my: 2 }} />

      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 2, mb: 2 }}>
        <Typography variant="h6">Sub Total:</Typography>
        <Typography variant="h6">Debits: {rows.reduce((sum, row) => sum + row.debit, 0)}</Typography>
        <Typography variant="h6">Credits: {rows.reduce((sum, row) => sum + row.credit, 0)}</Typography>
        ...
        <Typography
          variant="h6"
          sx={{ gridColumn: 'span 2', color: validateDebitsCredits() ? 'green' : 'red' }}
        >
          {rows.reduce((sum, row) => sum + row.debit, 0) - rows.reduce((sum, row) => sum + row.credit, 0)}
        </Typography>
      </Box>
      ...
      <Box sx={{ display: 'flex', gap: 2 }}>
        <Button variant="contained" color="success" disabled={!validateDebitsCredits()}>Save and Publish</Button>
